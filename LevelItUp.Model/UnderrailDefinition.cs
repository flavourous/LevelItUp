        ...
        public static GameBuild Generate(FakeDAL dal)
        {
            ...
            using (var builder = new DefinitionBuilder(dal, "Underrail", 25))
            {
                ...
                using (var feats = builder.ParameterType("Feats"))
                {
                    ...
                    f_armor_sloping = feats.Parameter("Armor Sloping").Require(01, t_level).Require(06,a_int).Require(15, s_mechanics).Commit();
                    f_burglar = feats.Parameter("Burglar").Require(01, t_level).Require(7,a_dex).Require(15, s_lockpicking).OrRequire(15,s_hacking).Commit();
                    f_clothier = feats.Parameter("Clothier").Require(01, t_level).Require(7,a_int).Require(15,s_tailoring).Commit();
                    f_conditioning = feats.Parameter("Conditioning").Require(01, t_level).Require(5,a_con).Commit();
                    f_doctor = feats.Parameter("Doctor").Require(01, t_level).Require(15,s_biology).Commit();
                    f_fast_metabolism = feats.Parameter("Fast Metabolism").Require(01, t_level).Require(6,a_con).Commit();
                    f_gun_nut = feats.Parameter("Gun Nut").Require(01, t_level).Require(7,a_int).Require(15,s_mechanics).Commit();
                    f_gunslinger = feats.Parameter("Gunslinger").Require(01, t_level).Require(7, a_dex).Require(15, s_guns).Commit();
                    f_heavy_punch = feats.Parameter("Heavy Punch").Require(01, t_level).Require(5, a_str).Require(15, s_melee).Commit();
                    f_hit_and_run = feats.Parameter("Hit and Run").Require(01, t_level).Require(7,a_agi).Commit();
                    f_juggernaut = feats.Parameter("Juggernaut").Require(01, t_level).Require(7,a_str).Require(7,a_str).Commit();
                    f_marksman = feats.Parameter("Marksman").Require(01, t_level).Require(5,a_dex).Require(15,s_crossbows).Commit();
                    f_power_management = feats.Parameter("Power Management").Require(01, t_level).Require(7,a_int).Require(15,s_electronics).Commit();
                    f_skinner = feats.Parameter("Skinner").Require(01, t_level).Require(7,a_int).Require(15,s_tailoring).Commit();
                    f_sprint = feats.Parameter("Sprint").Require(01, t_level).Require(6,a_agi).Commit();
                    f_stoicism = feats.Parameter("Stoicism").Require(01, t_level).Require(7,a_wil).Require(7,a_con).Commit();
                    f_suppressive_fire = feats.Parameter("Suppressive Fire").Require(01, t_level).Require(6,a_per).Require(10,s_guns).Commit();
                    f_sure_step = feats.Parameter("Sure Step").Require(01, t_level).Require(5, a_agi).Commit();
                    f_survival_instincts = feats.Parameter("Survival Instincts").Require(01, t_level).Require(9, a_con).Commit();
                    f_thick_skull = feats.Parameter("Thick Skull").Require(01, t_level).Require(10,a_con).Commit();

                    f_corporeal_projection = feats.Parameter("Corporeal Projection").Require(02, t_level).Require(7,a_str).Commit();
                    f_disassemble = feats.Parameter("Disassemble").Require(02, t_level).Require(7, a_int).Require(20, s_electronics).OrRequire(20, s_mechanics).Commit();
                    f_interloper = feats.Parameter("Interloper").Require(02, t_level).Require(7,a_agi).Require(20,s_stealth).Commit();
                    ...
                    f_pummel = feats.Parameter("Pummel").Require(02, t_level).Require(20,s_melee).Commit();
                    f_yell = feats.Parameter("Yell").Require(02, t_level).Require(20,s_intimidation).Commit();

                    f_cerebral_trauma = feats.Parameter("Cerebral Trauma").Require(04, t_level).Require(25,s_thoughtcontrol).Commit();
                    f_dirty_kick = feats.Parameter("Dirty Kick").Require(04, t_level).Require(25,s_melee).Commit();
                    f_force_user = feats.Parameter("Force User").Require(04, t_level).Require(7,a_wil).Require(25,s_psychokinesis).Commit();
                    f_high_technicalities = feats.Parameter("High-Technicalities").Require(04, t_level).Require(5,a_int).Require(25,s_guns).Commit();
                    f_kneecap_shot = feats.Parameter("Kneecap Shot").Require(04, t_level).Require(4,a_per).Require(25,s_guns).OrRequire(25,s_crossbows).Commit();
                    f_lightning_punches = feats.Parameter("Lightning Punches").Require(04, t_level).Require(8,a_dex).Require(25,s_melee).Commit();
                    f_point_shot = feats.Parameter("Point Shot").Require(04, t_level).Require(6,a_per).Require(6,a_dex).Require(25, s_guns).Commit();
                    f_quick_tinkering = feats.Parameter("Quick Tinkering").Require(04, t_level).Require(25,s_traps).Require(7,a_dex).Commit();
                    f_steadfast_aim = feats.Parameter("Steadfast Aim").Require(04, t_level).Require(5,a_str).Require(6,a_dex).Require(25,s_guns).Commit();
                    f_vile_weaponry = feats.Parameter("Vile Weaponry").Require(04, t_level).Require(10,s_biology).Require(25,s_crossbows).OrRequire(25,s_melee).OrRequire(25,s_traps).Commit();
                    f_weaponsmith = feats.Parameter("Weaponsmith").Require(04, t_level).Require(6,a_int).Require(25,s_mechanics).Commit();
                    f_bowyer = feats.Parameter("Bowyer").Require(04, t_level).Require(7,a_int).Require(30,s_mechanics).Commit();
                    f_concussive_shots = feats.Parameter("Concussive Shots").Require(04, t_level).Require(30,s_crossbows).Commit();
                    f_crippling_strike = feats.Parameter("Crippling Strike").Require(04, t_level).Require(30,s_melee).Commit();
                    f_escape_artist = feats.Parameter("Escape Artist").Require(04, t_level).Require(7,a_dex).Require(30,s_dodge).Commit();
                    f_grenadier = feats.Parameter("Grenadier").Require(04, t_level).Require(6,a_dex).Require(40,s_throwing).Commit();
                    f_meditation = feats.Parameter("Meditation").Require(04, t_level).Require(7,a_wil).Require(5,a_int).Require(1,f_tranquility).Require(50,s_thoughtcontrol).OrRequire(50,s_metathermics).OrRequire(50,s_psychokinesis).Commit();
                    f_mental_subversion = feats.Parameter("Mental Subversion").Require(04, t_level).Require(6,a_wil).Require(30,s_thoughtcontrol).Commit();
                    f_thermodynamicity = feats.Parameter("Thermodynamicity").Require(04, t_level).Require(30, s_metathermics).Commit();
                    f_trap_expert = feats.Parameter("Trap Expert").Require(04, t_level).Require(6,a_dex).Require(30,s_traps).Commit();
